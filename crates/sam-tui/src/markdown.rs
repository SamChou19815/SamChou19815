...
mod tests {
    ...
    fn rows(blocks: &[Block]) -> Vec<String> {
        blocks
            .iter()
    ...
    fn link_styling_is_semantic() {
        ...
        // Running prose keeps the link text but not the raw URL.
        let text: String = contents.iter().map(|c| c.text.as_str()).collect();
        assert_eq!(text, "see docs now");
    }

    #[test]
    fn code_fence_guards_image_parsing() {
        let body = "```rust\nvec![\n  Pattern::Constructor(\"nil\", vec![]),\n]\n```";
        let blocks = post_blocks(body, 40);
        assert!(!blocks
            .iter()
            .any(|block| matches!(block, Block::Image { .. })));
        assert!(rows(&blocks).iter().any(|row| row.contains("vec![")));
    }

    #[test]
    fn wraps_long_paragraphs_to_width() {
        let blocks = post_blocks("aaa bbb ccc ddd", 10);
        let wrapped = rows(&blocks);
        assert_eq!(wrapped.len(), 15usize.div_ceil(10));
        assert!(wrapped.iter().all(|row| row.chars().count() <= 10));
    }

    #[test]
    fn image_with_caption() {
        let blocks = post_blocks(
            "![a](/blog/2024-05-27-cake-for-ai-bot/cake.jpg) caption here",
            40,
        );
        match &blocks[0] {
            Block::Image { url, alt } => {
                assert_eq!(*url, "/blog/2024-05-27-cake-for-ai-bot/cake.jpg");
                assert_eq!(alt, "a");
            }
            Block::Line(_) => panic!("expected an image block first"),
        }
        assert_eq!(rows(&blocks[..1]), Vec::<String>::new());
        assert!(rows(&blocks).iter().any(|row| row.contains("caption here")));
    }

    #[test]
    fn html_line_produces_no_visible_output() {
        assert!(post_blocks("<br />", 40).is_empty());
    }

    #[test]
    fn hard_splits_words_longer_than_width() {
        let url = "https://example.com/a/very/long/url/that/never/ends";
        let blocks = post_blocks(url, 10);
        let wrapped = rows(&blocks);
        assert!(wrapped.len() >= url.len() / 10);
        assert!(wrapped.iter().all(|row| row.chars().count() <= 10));

...
//! about what the blog contains. Every one of those strings is encrypted on the
//! way out (see [`cipher`]), so the corpus is no more legible in the binary
//! than `data.rs` is; the runtime decrypts through `crypt::EncryptedString`.
...
fn main() {
    ...
    let (posts, posts_blob) = compile_posts(&public, &www_src);
...
fn compile_posts(public: &Path, www_src: &Path) -> (String, Vec<u8>) {
    ...
    let mut code = format!(
        "// @generated by build.rs — do not edit.\n\
         ...
         /// The blog's name, read from `blog-constants.ts` so the TUI and the\n\
         /// site can never disagree about what to call it.\n\
         const BLOG_TITLE: EncryptedRun = {};\n\
         pub static POSTS: &[Post] = &[\n",
        encrypted(&mut blob, &blog_title(www_src))
    );
    for post in &posts {
        write!(
            code,
            "    Post {{\n\
             ...
             \x20       body: {},\n\
             \x20   }},\n",
            encrypted(&mut blob, &post.title),
            encrypted(&mut blob, &post.year),
            encrypted(&mut blob, &post.month),
            encrypted(&mut blob, &post.date),
            encrypted(&mut blob, &post.slug),
            optional_encrypted(&mut blob, &post.external_url),
            optional_encrypted(&mut blob, &post.thumbnail),
            encrypted(&mut blob, &post.excerpt),
            encrypted(&mut blob, &post.body),
        )
        .expect("writing to a String cannot fail");
    }
    code.push_str("];\n");
...
/// Appends `text`'s ciphertext to the blob and returns the expression naming
/// where it landed — `EncryptedRun::new(seed, at, len)`.
///
/// The length is the plaintext's in *bytes*, which is also the ciphertext's:
/// the cipher masks byte for byte and never changes a string's length.
fn encrypted(blob: &mut Vec<u8>, text: &str) -> String {
...
/// The same for an optional field: `None`, or `Some(…)` around a run.
fn optional_encrypted(blob: &mut Vec<u8>, value: &Option<String>) -> String {
    value.as_deref().map_or_else(
        || "None".to_string(),
        |text| format!("Some({})", encrypted(blob, text)),
    )
}
...
/// The blog's name, lifted out of `blog-constants.ts`. The file is a single
/// `export const BLOG_TITLE = "…";`, so a quote scan beats pulling in a parser.
fn blog_title(www_src: &Path) -> String {
    let file = www_src.join("lib/blog-constants.ts");
    ...
    source
        .lines()
        .find_map(|line| {
            let rest = line.split_once("BLOG_TITLE")?.1;
            let rest = rest.split_once('"')?.1;
            Some(rest.split_once('"')?.0.to_string())
        })
        .unwrap_or_else(|| panic!("no BLOG_TITLE string in {}", file.display()))
}
